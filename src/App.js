    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0a0a0a', color: '#e0e0e0', fontFamily: "'DM Mono',monospace" }}>
      ...
      <div style={{ width: 220, background: '#0d0d0d', borderRight: '1px solid #1a1a1a', display: 'flex', flexDirection: 'column', padding: '28px 0', position: 'sticky', top: 0, height: '100vh', flexShrink: 0 }}>
        ...
        <nav style={{ flex: 1, padding: '16px 12px' }}>
          {navItems.map(item => (
            <button key={item.key} onClick={() => setPage(item.key)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderRadius: 10, border: 'none', cursor: 'pointer', background: page === item.key ? '#4ade8015' : 'transparent', color: page === item.key ? '#4ade80' : '#555', fontFamily: "'DM Mono',monospace", fontSize: 12, marginBottom: 2, textAlign: 'left' }}>
